...
export class Board {
  private readonly _walledDiscs: Disc[][];

  constructor(private readonly _discs: Disc[][]) {
  ...
  private listFlipPoints(move: Move): Point[] {
    ...
    // 指定した方向にひっくり返せる石がいくつあるかチェック
    const checkFlipPoints = (xMove: number, yMove: number): void => {
  ...
  private wallDiscs(): Disc[][] {
    ...
    // Wallの値が10(8+2)個入った配列を作成
    const topAndBottomWall = new Array(this._discs[0].length + 2).fill(
      Disc.Wall,
    );
